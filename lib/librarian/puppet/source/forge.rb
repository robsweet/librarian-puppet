      ...
      class Forge
        class Repo
          ...
          def cache_version_unpacked!(version)
            ...
            command = "puppet module install --http_proxy_host '#{proxy.host}' --http_proxy_port '#{proxy.port}'  --version #{version} --target-dir '#{path}' --module_repository '#{source}' --modulepath '#{path}' --ignore-dependencies '#{target}'"
            output = `#{command}`
